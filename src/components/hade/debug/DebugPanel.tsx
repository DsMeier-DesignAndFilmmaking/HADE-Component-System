...
function IntentSection({ probs }: { probs: Record<string, number> }) {
  ...
  return (
    <div className="px-4 py-3 border-b border-line/30">
      <p className="text-[9px] uppercase tracking-widest text-ink/65 mb-2.5">
        Intent Distribution
      </p>
      <div className="space-y-1.5">
        {entries.map(([intent, prob]) => (
          <div key={intent} className="flex items-center gap-2">
            <span className="w-14 text-[10px] text-ink/65 capitalize shrink-0">
              {intent}
            </span>
  ...
  return (
    <div className="px-4 py-3 border-b border-line/30">
      <div className="flex items-center justify-between mb-2">
        <p className="text-[9px] uppercase tracking-widest text-ink/65">
          Candidates
        </p>
        <span className="text-[9px] text-ink/60 font-mono">P · I · C → final</span>
      </div>
      <div className="space-y-0.5">
        {candidates.map((c, i) => (
          ...
          >
            {/* Rank */}
            <span className="text-[9px] text-ink/60 w-4 shrink-0 tabular-nums">
              {i + 1}.
            </span>
            {/* Name */}
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] uppercase tracking-widest text-ink/65 font-mono">
            Decision Reasoning
          </span>
        </div>
        <span
          className={`text-[10px] text-ink/65 transition-transform duration-200 ${
