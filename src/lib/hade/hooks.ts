...
export function useAdaptive(config: HadeConfig = {}): AdaptiveState {
  ...
  function generateLocalAlternative(state: {
    ...
    const decision: HadeDecision = {
      ...
      why_this: "Local fallback while we work to reconnect.",
      decision_frame: "Operating offline — surfacing the safest local pick.",
      confidence: 0.4,
    ...
    async (req?: Partial<DecideRequest>) => {
      ...
      try {
        ...
        const safeDecision: HadeDecision = {
          ...
          confidence_label: dec?.confidence_label ?? "Exploratory",
      ...
      if (decision.is_fallback) {
        ...
        if (recoveryPlace) {
          ...
          const recovered: HadeDecision = {
            ...
            why_this: "Real nearby spot recovered from your cache.",
            decision_frame: "Swapping to a real cached venue — fewer surprises.",
            confidence: 0.5,
            confidence_label: "Exploratory",
