...
function detectSchemaVersion(raw) {
  if (!isPlainObject2(raw)) return "unknown";
...
function createHade(clientConfig = {}) {
  ...
  let lastOutput = null;
  async function decide2(input, options) {
  ...
  async function refine(input, prior, options) {
    ...
    return decide2(refinedInput, options);
  }
  function getConfig() {
...
export { BUILT_IN_DOMAINS, BUILT_IN_SCORING_PROFILES, CURRENT_SCHEMA_VERSION, DECISION_ENGINE_OUTPUT_VERSION, DEFAULT_CONFIG_HASH, DEFAULT_HADE_CONFIG, HADE_CORE_VERSION, HadeConfigValidationError, HadeError, SURFACED_ONCE_PENALTY, SURFACED_TWICE_PENALTY, VIBE_TAG_SENTIMENT, assertValidConfig, buildDecisionEngineOutput, buildExplanation, compositeGeo, computeConfidence, computeConfigHash, computeConfigHashSync, computeSurfacedPenalty, confidenceBand, confidenceLabelId, createHade, createHadeErrorFactory, createVenueAdapter, decide, defineConfig, detectSchemaVersion, emptyVenues, extractRejectedVenueIds, extractSurfacedFallbackTitles, formatDistance, formatEta, fromDecideResponse, fromHadeDecision, getVenueAdapter, headerGeo, ipLookupGeo, isHadeError, legacyOpenAIAdapter, legacyUpstashAdapter, loadConfig, memoryCache, migrateConfig, noopLLM, normalizeDecisionSource, recoverLeastRecentlySurfaced, registerDefaultAdapters, resetAdapterRegistryForTests, resolveAdapters, resolveEffectiveCopy, setDefaultVenueAdapterFactory, sortFallbackCandidates, staticGeo, syntheticConfidence, unwrappedGooglePlaces, validateConfig };
