...
import { DOMAIN_CATEGORY_BUCKETS, DOMAIN_RADIUS_M } from "@/core/services/places";
...
export async function generateSyntheticDecision(
  ...
  try {
    ...
    const venueAdapter = resolveAdapters().venue;
    try {
      places = (lensBuckets && lensBuckets.length > 0 && geoHint
        ? await venueAdapter.searchMultiQuery({
            geo: geoHint,
            categoryBuckets: lensBuckets,
            radius_meters: domainRadius,
          })
        : domainBuckets && geoHint
          ? await venueAdapter.searchMultiQuery({
              geo: geoHint,
              categoryBuckets: domainBuckets,
              radius_meters: domainRadius,
            })
          : await getPlacesCandidates(ctx, categories)) as PlaceOption[];
    } catch (err) {
