...
import { resolveAdapters } from "@hade/core";
...
async function buildFallbackCandidates(
  ...
  if (geo && geoSource !== "unknown") {
    try {
      ...
      const places = await resolveAdapters().venue.searchNearby({
        geo,
        radius_meters: RADIUS.SEARCH_DEFAULT,
        open_now: true,
      });
...
async function enhanceCopyWithLLM(
  ...
  // Phase E: transport moved to LLMAdapter (registered in registerDefaults.ts).
  // This function keeps ownership of: prompt construction, extractSafeCopyPatch
  // (anti-LLM-swap safety guard), per-field char caps, and reqId-prefixed logs.
  if (!serverEnv.openAiApiKey) return null;

  const llm = resolveAdapters().llm;
  if (!llm) {
    console.warn(`[hade-copy-enhance ${reqId}] ✗ no LLM adapter registered`);
    return null;
  }
  ...
  // Byte-identical to the legacy user content shape (was inline at route.ts:835-845).
  ...
  let patch: Awaited<ReturnType<typeof llm.enhanceCopy>>;
  try {
    patch = await llm.enhanceCopy(userContent, { timeout_ms: COPY_ENHANCE_TIMEOUT_MS });
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err);
  ...
  if (!patch) {
    // The adapter swallows transport/parse failures and returns null — preserves
    // the legacy contract that null means "use deterministic copy unchanged".
    console.warn(`[hade-copy-enhance ${reqId}] ✗ LLM returned no patch`);
    return null;
  }

  const safe = extractSafeCopyPatch(decision.id, patch as Record<string, unknown>, reqId);
  if (!safe) return null;
  ...
  const validated: Partial<typeof safe> = {};
  if (safe.rationale      && safe.rationale.length      <=  280) validated.rationale      = safe.rationale;
  if (safe.why_now        && safe.why_now.length        <=  120) validated.why_now        = safe.why_now;
  if (safe.why_this       && safe.why_this.length       <=   60) validated.why_this       = safe.why_this;
  if (safe.decision_frame && safe.decision_frame.length <=  180) validated.decision_frame = safe.decision_frame;
